...
```ts
  ...
  120 | 
  121 |     // Chỉ verify option đó không còn trong dropdown list
  122 |     // (đã bị remove vì đã được chọn)
> 123 |     await this.page.locator('p:has-text("Genre") + div [class*="-control"]').click();
      |                ^ TypeError: Cannot read properties of undefined (reading 'page')
  124 |     
  125 |     const option = homePage.page.locator('[class*="-menu"]')
  126 |       .getByText(genre.label, { exact: true });
  127 |     
  128 |     await expect(option).not.toBeVisible();
  129 |     logger.info(`Genre "${genre.label}" selected and removed from list ✅`);
  130 | 
  131 |     // Đóng dropdown
  132 |     await homePage.page.keyboard.press('Escape');
  133 |     await homePage.page.waitForTimeout(300);
  134 |   }
  135 | });
  136 | 
  137 |   // ── TC-GNR-08 (API) ────────────────────────────────────────────────────────
  138 |   test('TC-GNR-08: Selecting a genre triggers an API call with correct genre param', async () => {
  139 |     logger.step('Collect API calls when selecting "Thriller" genre');
  140 | 
  141 |     const calls = await homePage.collectApiCalls(async () => {
  142 |       await homePage.selectGenre(GENRES.THRILLER);
  143 |     });
  144 | 
  145 |     expect(calls.length).toBeGreaterThan(0);
  146 | 
  147 |     const firstCall = calls[0];
  148 |     logger.info(`API endpoint: ${firstCall.url.pathname}`);
  149 |     logger.info(`API params: ${firstCall.url.searchParams.toString()}`);
  150 | 
  151 |     // The API call should include genre-related parameter
  152 |     const hasGenreParam =
  153 |       firstCall.url.searchParams.has('with_genres') ||
  154 |       firstCall.url.searchParams.has('genre');
  155 | 
  156 |     logger.info(`API includes genre param: ${hasGenreParam}`);
  157 |     await homePage.page.screenshot({
  158 |       path: 'test-results/screenshots/tc-gnr-08-api-genre.png',
  159 |     });
  160 |   });
  161 | });
```