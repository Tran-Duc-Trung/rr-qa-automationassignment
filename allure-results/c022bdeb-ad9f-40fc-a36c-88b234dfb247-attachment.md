...
- Name: filtering\search.spec.js >> Search Filter >> TC-SRC-05: Search with partial title returns relevant results
- Location: tests\filtering\search.spec.js:81:3
  ...
  - generic [ref=e4]:
    - banner [ref=e5]:
      ...
      - generic [ref=e18]:
        ...
        - textbox "SEARCH" [active] [ref=e20]: Ava
    - generic [ref=e22]:
      - generic [ref=e23]:
        - generic [ref=e24]:
...
> 94  |     expect(allMatch).toBe(true);
  ...
  144 |     await expect(nav).toBeVisible();
  145 | 
  146 |     const cardCount = await homePage.getCardCount();
  147 |     logger.info(`Cards after special char search: ${cardCount}`);
  148 | 
  149 |     await homePage.page.screenshot({
  150 |       path: 'test-results/screenshots/tc-src-08-special-chars.png',
  151 |     });
  152 |   });
  153 | });
```